...
const FilterSex = () => {
    ...
    if (sex) {
      ...
      try {
        ...
        const queries = tables.map((table) =>
          supabase.from(table).select("*").eq("sex", sex)
        );
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {selectedSex && !loading && (
        <div className="mb-4">
          <h2 className="text-xl font-semibold">
            Nigerians ({selectedSex}): {filteredData.length} results
          </h2>
        </div>
      )}
      ...
      {selectedSex && !loading && filteredData.length === 0 && (
        <p className="text-gray-600">No results found for {selectedSex}.</p>
      )}
    </div>
  );
};
