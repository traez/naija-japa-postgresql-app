...
//import data from "@/lib/data100uk.json";
...
const NaijaUk = () => {
  ...
  if (loading) {
    return (
      <div className="text-xl font-semibold my-4 px-4">
        Loading Nigerians in UK...
      </div>
    );
  }

  if (!s4data.s4uk.length) {
    return (
      <div className="text-xl font-semibold my-4 px-4">
        No data available for Nigerians in UK.
      </div>
    );
  }
  ...
  return (
    <>
      <div className="text-xl font-semibold my-4 px-4">Nigerians in UK</div>
      <section className="grid grid-cols-[repeat(auto-fit,_minmax(165px,_1fr))] gap-4">
      {s4data.s4uk.map((item, index) => (
          <article
